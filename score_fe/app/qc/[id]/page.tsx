...
export default function QcViewerPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/bff/shots/${qcId}/tasks`, { headers })
      .then(async (res) => {
    ...
    >
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        ...
        <nav className="text-xs text-slate-500 px-8 pb-2" aria-label="breadcrumb">
          ...
          <span className="text-indigo-600 font-bold">{t('qc.detail.title')}</span>
          {qcId && (
            <>
              <span className="mx-1">›</span>
              <span className="text-slate-600 font-bold" translate="no">#{qcId}</span>
            </>
          )}
        </nav>
        ...
        <div className="flex-1 overflow-y-auto p-10 space-y-8">
          ...
          <div className="bg-white/40 backdrop-blur-xl border border-white/60 rounded-3xl shadow-xl p-6">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-2xl bg-violet-100 flex items-center justify-center text-3xl shrink-0">
                🔍
              </div>
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-widest">QC ID</p>
                <h2 className="text-2xl font-black text-slate-800" translate="no">#{qcId}</h2>
              </div>
            </div>
          </div>
          ...
          <section>
            <h2 className="text-lg font-black text-slate-800 mb-5 flex items-center gap-2">
              <span className="w-2 h-6 bg-violet-400 rounded-full inline-block" />
              📋 {t('qc.detail.tasks')}
            </h2>
              ...
              {tasks.map((task) => {
                ...
                return (
                  <div
                    ...
                    className="bg-white/40 backdrop-blur-md border border-white/60 rounded-3xl shadow-xl p-5 flex flex-col gap-2"
                  >
                    <div className="flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs text-slate-400 uppercase tracking-widest" translate="no">
                            Task #{taskId}
                          </p>
                          {task.type && (
                            <p className="text-sm font-bold text-slate-700" translate="no">
                              {task.type}
                            </p>
                          )}
                          {task.name && (
