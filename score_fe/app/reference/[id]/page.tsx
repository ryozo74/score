...
export default function ReferenceViewerPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/bff/shots/${referenceId}/tasks`, { headers })
      .then(async (res) => {
    ...
    >
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        ...
        <nav className="text-xs text-slate-500 px-8 pb-2" aria-label="breadcrumb">
          ...
          <span className="text-indigo-600 font-bold">{t('detail.title')}</span>
          {referenceId && (
            <>
              <span className="mx-1">›</span>
              <span className="text-slate-600 font-bold" translate="no">#{referenceId}</span>
            </>
          )}
        </nav>
        ...
        <div className="flex-1 overflow-y-auto p-10 space-y-8">
          ...
          <div className="bg-white/40 backdrop-blur-xl border border-white/60 rounded-3xl shadow-xl p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-widest">Reference ID</p>
                <h2 className="text-2xl font-black text-slate-800" translate="no">#{referenceId}</h2>
              </div>
            </div>
          </div>
